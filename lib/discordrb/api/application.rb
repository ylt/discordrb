...
module Discordrb::API::Application
  ...
  def create_global_command(token, application_id, name, description, options = [], default_permission = nil, type = 1, integration_types=nil, contexts=nil)
    Discordrb::API.request(
      :applications_aid_commands,
      nil,
      :post,
      "#{Discordrb::API.api_base}/applications/#{application_id}/commands",
      { name: name, description: description, options: options, default_permission: default_permission, type: type, integration_types: integration_types, contexts: contexts }.compact.to_json,
      Authorization: token,
      content_type: :json
    )
  end
  ...
  def edit_global_command(token, application_id, command_id, name = nil, description = nil, options = nil, default_permission = nil, type = 1, integration_types=nil, contexts=nil)
    Discordrb::API.request(
      :applications_aid_commands_cid,
      nil,
      :patch,
